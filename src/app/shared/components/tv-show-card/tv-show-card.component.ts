...
@Component({
  ...
  styleUrl: './tv-show-card.component.scss',
  animations: [
    trigger('fadeIn', [
      transition(':enter', [
        style({ opacity: 0 }),
...
export class TvShowCardComponent implements OnInit {
  ...
  get title(): string {
    return this.type === 'tv' ? (this.item as Movie).title : (this.item as TvShow).name;
  }

  get releaseDate(): string {
    const dateString = this.type === 'tv' ? (this.item as Movie).release_date : (this.item as TvShow).first_air_date;
